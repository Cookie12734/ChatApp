...
現在は、ユーザー登録・ログイン・メール認証・フレンド申請・通知機能・プロフィール設定・フレンドとの個人チャット・サーバー形式のグループチャットなどを実装しています。  
今後は、メッセージ管理、ブロック機能、プロフィール詳細ページなどを追加し、Discordのようにユーザー同士が安全に交流できるWebアプリケーションを目指しています。
